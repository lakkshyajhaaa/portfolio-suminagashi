...
export default function Work() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-8 md:p-24 bg-transparent">
      ...
      <div className="absolute top-[120px] left-6 md:top-24 md:left-32 flex flex-col pointer-events-auto z-10 mix-blend-difference animate-fade-in">
      ...
      <div className="absolute top-[260px] md:top-[120px] right-6 md:right-16 flex flex-col items-end gap-4 md:gap-8 pointer-events-auto z-10 mix-blend-difference animate-fade-in w-full max-w-[80vw] pb-24">
        {projects.map((project) => (
          <Link key={project.id} href={`/work/${project.slug}`} className="flex flex-col items-end group cursor-pointer transition-transform duration-500 hover:-translate-x-2 md:hover:-translate-x-8">
            <div className="flex flex-col md:flex-row items-end md:items-baseline gap-1 md:gap-8">
              ...
              <h2 className="font-sans text-[12vw] md:text-[6rem] lg:text-[7.5rem] tracking-[-0.05em] font-bold text-white/90 leading-[0.85] drop-shadow-[0_0_15px_rgba(255,255,255,0.2)] group-hover:text-[#f3e8ff] group-hover:drop-shadow-[0_0_25px_rgba(243,232,255,0.6)] transition-all duration-300 whitespace-nowrap">
                {project.name}
              </h2>
            </div>
